...
class CI_Router extends Router {
	...
	function _set_routing()
	{
		...
		@include(FCPATH.'config/routes'.EXT);
	...
	function _set_request($segments = array())
	{
		...
		if(count($segments) == 1) {
			...
			if(file_exists(FCPATH.'controllers/'.$segments[0].'/'.$segments[0].EXT)) {
			...
			for($i=0 ; $i < $segNum ; $i++) {
				...
				} elseif(isset($segments[0]) && file_exists(FCPATH.'controllers/'.implode('/',$dir).'/'.$segments[0])) {

					continue;

				// exite un archivo que coincide con el segmento
				} elseif( file_exists(FCPATH.'controllers/'.implode('/',$dir).EXT)) {
				...
				} elseif (file_exists(FCPATH.'controllers/'.implode('/',$dir).'/'.$currentParam.EXT) ) {
