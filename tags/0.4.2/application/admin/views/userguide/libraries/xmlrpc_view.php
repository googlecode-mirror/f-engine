...
<p>
    F-engine's XML-RPC classes permit you to send requests to another server, or set up
    your own XML-RPC server to receive requests.
</p>
<h2>What is XML-RPC?</h2>
<p>
    Quite simply it is a way for two computers to communicate over the internet using XML. 
    One computer, which we will call the 
    <dfn>
        client
    </dfn>, sends an XML-RPC <strong>request</strong>
    to 
    another computer, which we will call the 
    <dfn>
        server
    </dfn>.  Once the server receives and processes the request it 
    will send back a <strong>response</strong>
    to the client.
</p>
<p>
...
<p>
    For detailed specifications, you can visit the <a href="http://www.xmlrpc.com/">XML-RPC</a>
    site.
</p>
<h2>Initializing the Class</h2>
<p>
    Like most other classes in F-engine, the XML-RPC and XML-RPCS classes are initialized in your controller using the 
    <dfn>
        $this->load->library
    </dfn>
    function:
...
<p class="important">
    <strong>Note:</strong>&nbsp; When using the XML-RPC Server class you must load BOTH the XML-RPC class and the XML-RPC Server class.
</p>
...
<p>
    Here is a basic example that sends a simple Weblogs.com ping to the <a href="http://pingomatic.com/">Ping-o-Matic</a>
</p>
<code>
    $this->load->library('xmlrpc');
    <br/>
...
<p>
    ...
    Lastly, the full request is sent.  If the 
    <dfn>
        send_request()
    </dfn>
    method returns false we will display the error message
...
<p>
    ...
    is simply the data you are sending to the XML-RPC server.  Each piece of data in a request 
    is referred to as a 
    <dfn>
        request parameter
    </dfn>.  The above example has two parameters:
...
<p>
    ...
    you will have to include the data type in the request array.
</p>
<p>
...
<p>
    ...
    each parameter into its own array, with the data type in the second position:
</p>
<code>
    $request = array (
    <br/>
...
The <a href="#datatypes">Data Types</a>
section below has a full list of data types.<h2>Creating an XML-RPC Server</h2>
<p>
    An XML-RPC Server acts as a traffic cop of sorts, waiting for incoming requests and redirecting them to the
    appropriate functions for processing.
</p>
<p>
    To create your own XML-RPC server involves initializing the XML-RPC Server class in your controller where you expect the incoming
    ...
    class and method for processing.
</p>
<p>
...
<p>
    ...
    <var>
        object
    </var>' key is a special key that you pass an instantiated class object with, which is necessary when the method you are mapping to is not
    part of the F-engine super object.
</p>
<p>
    In other words, if an XML-RPC Client sends a request for the 
    <var>
        new_post
    </var>
    method, your 
    server will load the 
    <dfn>
        My_blog
    </dfn>
    class and call the 
...
<p>
    ...
    an object to that method containing the data sent by the client.
</p>
<p>
    ...
    method is requested, the server will expect a class
...
<p>
    ...
    variable is an object compiled by the Server, which contains the data sent by the XML-RPC Client. 
    Using this object you will have access to the <em>request parameters</em>
    enabling you to process the request.  When 
    you are done you will send a 
    <dfn>
        Response
    </dfn>
    back to the Client.
...
<p>
    ...
    function retrieves an indexed array corresponding to the request parameters sent by the client.
    In the above example, the output parameters will be the username and password.
</p>
<p>
...
<p>
    Similar to <em>Requests</em>, <em>Responses</em>
    must be formatted as an array.  However, unlike requests, a response is an array<strong>that contains a single item</strong>.  This item can be an array with several additional arrays, but there
...
<p class="important">
    Notice that the above array is formatted as a 
    <dfn>
        struct
    </dfn>.  This is the most common data type for responses.
</p>
<p>
    As with Requests, a response can be one of the seven data types listed in the <a href="#datatypes">Data Types</a>
    section.
</p>
...
<textarea class="textarea" style="width:100%" cols="50" rows="32">
&lt;?php
...
<p>
    Note: In the above code we are using a "url helper".  You can find more information in the <a href="../general/helpers.html">Helpers Functions</a>
    page.
</p>
<h3>The Server</h3>
<p>
...
<p>
    The client you created sends a message ("How's is going?") to the server, along with a request for the "Greetings" method.
    The Server receives the request and maps it to the "process" function, where a response is sent back.
</p>
...
<p>
    This function lets you send an error message from your server to the client. First parameter is the error number while the second parameter
...
<h2>Data Types</h2>
<p>
    According to the <a href="http://www.xmlrpc.com/spec">XML-RPC spec</a>
    there are seven types
    of values that you can send via XML-RPC:
</p>
<ul>
    <li>
        <em>int</em>
